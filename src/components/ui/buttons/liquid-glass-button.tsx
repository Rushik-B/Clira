...
export type LiquidButtonHdrGlow = {
  srgb: HdrGlowColors;
  p3?: HdrGlowColors;
};

type StyleWithCssVars = React.CSSProperties & Record<`--${string}`, string | number>;

export const LIQUID_BUTTON_BASE_CLASS = 'rounded-2xl px-5 text-sm font-semibold text-sky-100';
...
function LiquidButton({
  ...
  const resolvedHdrGlow: LiquidButtonHdrGlow | undefined = hdrHover
    ? (hdrGlow ?? {
        srgb: {
          primary: "rgba(56, 189, 248, 0.70)",
          secondary: "rgba(16, 185, 129, 0.60)",
        },
        p3: {
          primary: "color(display-p3 0.0 0.78 1.0 / 0.80)",
          secondary: "color(display-p3 0.0 1.0 0.60 / 0.75)",
        },
      })
    : undefined;

  const hdrVars: StyleWithCssVars | undefined = resolvedHdrGlow
    ? {
        ["--hdr-hover-glow-1-srgb"]: resolvedHdrGlow.srgb.primary,
        ["--hdr-hover-glow-2-srgb"]: resolvedHdrGlow.srgb.secondary,
        ...(resolvedHdrGlow.p3
          ? {
              ["--hdr-hover-glow-1-p3"]: resolvedHdrGlow.p3.primary,
              ["--hdr-hover-glow-2-p3"]: resolvedHdrGlow.p3.secondary,
            }
          : {}),
      }
    : undefined;

  const mergedStyle: React.CSSProperties | undefined = hdrVars
    ? ({ ...(style ?? {}), ...hdrVars } satisfies StyleWithCssVars)
    : style;
    ...
    <>
      <Comp
        ...
        className={cn(
          ...
          minWidth ? minWidthClasses[minWidth] : undefined,
          hdrHover && "hdr-hover-glow",
