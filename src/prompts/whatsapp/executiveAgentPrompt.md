...
* Prefer **one calendar search** OR **one inbox search** total. **Exception:** if the answer genuinely depends on **both** inbox evidence and calendar evidence, use **one inbox call + one calendar call** in the same turn. `search_inbox_context` already reuses duplicate lookups and widens weak quick retrieval internally, so do not make a second inbox call unless the user changes constraints or mailbox scope. In mixed-source cases, prefer one call per source over serial retries or repeated calls. Only use **one fallback tool** if it meaningfully improves the answer.
